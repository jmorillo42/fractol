...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/20 23:09:57 by jmorillo          #+#    #+#             */
/*   Updated: 2022/11/28 18:13:04 by jmorillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	fr_julia(t_fractal *fractal)
{
	...
	p = fractal->point;
	cx_set(p->z, ((double)p->x * fractal->range->r / fractal->width)
		+ fractal->offset->r,
		((double)p->y * fractal->range->i / fractal->height)
		+ fractal->offset->i);
	cx_set(p->z2, p->z->r * p->z->r, p->z->i * p->z->i);
	iteration = 0;
